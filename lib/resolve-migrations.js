...
/** @import { AnyUmzeptionContext, UmzeptionDefinition, UmzeptionDependency } from './advanced-types.d.ts' */
 ...
 * check (`Set(result).size === input.length`) relies on this — callers
 * inside this module use globby's output, which dedupes by design. A caller
 * passing already-duplicated input would see an "returned duplicate entries"
 * error even for a faithful identity result, which would be misleading.
 *
 * @param {string[]} input
...
export function validateSortResult (input, result, definitionName) {
  ...
  // Catch the duplicate-replacing-unique case: [a,b,c] -> [a,b,b]. The
  // membership loop alone accepts it (length matches, every entry is from
  // input); only checking that the result has the same number of *distinct*
  // entries as input rules it out. globby's output is dup-free, so this is
  // sufficient — pigeonhole then guarantees a permutation.
 ...
 * @param {Pick<UmzeptionDefinition<T>, 'glob' | 'name' | 'noPrefix' | 'pluginDir'>} definition
 * @param {T} _context
 * @param {boolean} [noop] - if set then the up/down migrations will have no operation
 * @param {UmzeptionDependency['sortFiles']} [sortFiles] - custom sort transformer; defaults to {@link sortMigrationFiles}
