...
export function loadPlugins (processPlugin, options = {}) {
  ...
  return async (pluginName) => {
    ...
    const relativePluginPath = '.' + path.sep + path.relative(dirname, pluginPath);

    /** @type {unknown} */
    const loadedPlugin = await import(relativePluginPath)
      .catch(cause => {
        throw new Error(`Import of ${pluginName} from ${relativePluginPath} failed`, { cause });
      });
