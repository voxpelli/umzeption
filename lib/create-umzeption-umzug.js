 ...
 * @param {UmzeptionLookupOptions<T>} options.umzeption - umzeption config (dependencies, glob, sortFiles, install, meta/cwd, ...)
 ...
 * // tools/umzug.js
 * import pg from 'pg';
 * import { createUmzeptionUmzug, createUmzeptionPgContext, UmzeptionPgStorage } from 'umzeption';
 *
 * const umzug = createUmzeptionUmzug({
 *   umzeption: { dependencies: ['@yikesable/foo'], meta: import.meta },
 *   context: createUmzeptionPgContext(new pg.Pool({ connectionString: process.env.DATABASE_URL })),
 *   storage: new UmzeptionPgStorage(),
 * });
 *
 * umzug.runAsCLI(); // node tools/umzug.js {up,down,pending,create}
 */
